...
<div class="space-y-4">
  ...
  <div class="flex items-center gap-3">
    ...
    <input id="days" type="number" min="0" class="border border-zinc-300 dark:border-ios-border rounded-lg px-2 py-1 w-24 bg-white dark:bg-ios-surface text-zinc-900 dark:text-ios-text focus:outline-none focus-visible:ring-2 focus-visible:ring-sky-400 dark:focus-visible:ring-sky-300" on:input={onInput} bind:value={$totalDays} />
  </div>
  ...
  <div class="flex items-center gap-2">
    ...
    <span class="badge border-zinc-200 dark:border-ios-border text-zinc-700 dark:text-zinc-300">{ $locale === 'de-DE' ? 'Gewählt' : 'Selected' }: {$vacationDays.size}</span>
  </div>
  ...
  <div>
    ...
    {:else}
      <ul class="max-h-48 overflow-auto divide-y divide-zinc-200 dark:divide-ios-border text-sm">
        {#each Array.from($vacationDays).sort() as d}
          <li class="flex items-center justify-between py-1">
            <span class="text-zinc-800 dark:text-ios-text">{new Date(d).toLocaleDateString($locale)}</span>
